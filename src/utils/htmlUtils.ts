...
export function replaceUrl(html: string, baseDir: string) {
  ...
  // const regex = /src="[^"]*\/([^"\/?]+)"/g;
  const regex = /src="[^"]*\/([^"?]+)[^"]*"/g;
  const regexAStart = /<a href="javascript:void\(0\);">/g;
  const regexAEnd = /<\/a>/g;
  let newHtml = html;
  if (isPackaged) {
    newHtml = newHtml.replaceAll(regex, `src="${baseDir}/$1"`)
  } else {
    newHtml = html.replaceAll(regex, (match, p1) => {
  ...
  newHtml = newHtml.replaceAll(regexAStart, '')
  return newHtml.replaceAll(regexAEnd, '')
}