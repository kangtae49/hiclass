            ...
            with open(commentJson, 'r', encoding='utf-8') as f:
                ...
                for post in posts:
                    ...
                    if files is None:
                        continue
                    for file in files:
                        content_type = file['fileContentType']
                        if content_type.startswith('image/'):
                            attach_post_url = file['fileOriginalPath']
                        else:
                            attach_post_url = file['fileTranscodePath']
                        if attach_post_url is None:
                            continue
                        # print(attach_post_url)
                        pure_path = urlparse(attach_post_url).path
                        file_name = os.path.basename(pure_path)
                        save_name = f'{attach_dir}/{file_name}'
                        # print('save_name:', save_name)
                        download_get(attach_post_url, save_name, {})
                # url_post_attach = f"https://api.hiclass.net/postComments/!q?postId={postId}&depth=0&del=false&page=0&size=100&sort=insertedTimestamp%2"
